...
def test_managed_diff_preserves_unrelated_machine_policy():
    """managed_config_diff merges only the OSH-owned fleet-gates settings; unrelated
    managed keys (an operator's approval/security policy) are preserved."""
    inst = _installer()
    existing = {
...
def test_managed_installed_requires_expected_ssh_host():
    """A pre-existing FLEET-F62 managed fragment (profile_roles, no expected_ssh_host)
    is NOT treated as installed, so the openshell managed write still runs (veto not left
    fail-closed)."""
    import os, yaml
    inst = _installer()
    ...
    (managed / "config.yaml").write_text(yaml.safe_dump(base), encoding="utf-8")
    assert inst.collect_home_state(spec, str(_spec_path()))["managed_installed"] is False
    base["plugins"]["entries"]["nv-fleet-gates"]["settings"]["expected_ssh_host"] = {"builder": "osh-f64-builder"}
    (managed / "config.yaml").write_text(yaml.safe_dump(base), encoding="utf-8")
    assert inst.collect_home_state(spec, str(_spec_path()))["managed_installed"] is True
