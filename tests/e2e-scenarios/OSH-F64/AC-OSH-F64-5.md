...
`openshell forward start osh-f64-gw <local>:<gateway port>` (or `openshell service expose`)
...
down): `openshell sandbox create --name osh-f64-gw --from <gateway image>` (the same pinned
Hermes-runtime image AC-3 names), start its Hermes gateway and capture its loopback
credential URL as `$GW_URL`, then `install-into-sandbox.sh <spec> --ref <sha> --gateway-url
"$GW_URL"` (a real run requires `--gateway-url`), boot serving `default` + ≥2
coworkers under a stub model; `openshell forward start osh-f64-gw <local>:<gateway port>`
establishes the local forward; the dashboard is reached at the forwarded address under
agent-browser (hermes-ui-driver skill). No fixture is re-installed here. Teardown deletes
`osh-f64-*` at the end.
