...
`pre_tool_call` fires before any tool executes and lets a plugin block or approve the call. The **single production invocation point** is `_dispatch_pre_tool_call_hooks` (`hermes_cli/plugins.py:6831`), which returns `(block_message, modified_args)`; the hook itself is emitted one level down in `_get_pre_tool_call_directive_details` via `invoke_lifecycle_hook("pre_tool_call", ...)` (`hermes_cli/plugins.py:6635`). Every tool executor routes through that one dispatcher — `agent/tool_executor.py:647-649`, `agent/agent_runtime_helpers.py:3475-3476`, and `model_tools.py:1435-1436` — so a gate registered once covers all of them.
...
2. **Neither session-start surface carries `chat_id`.** The colon `session:start` emit carries only `platform` / `user_id` / `session_id` / `session_key` (`gateway/run.py:20589-20594`, tag: `gateway/run.py:20589` \| main: `gateway/run_turn.py:379-384`), and the plugin `on_session_start` emit carries only `session_id` / `model` / `platform` (`agent/conversation_loop.py:1100-1105`) — also no `chat_id`. Do not infer chat context from either session-start payload; a plugin that needs `chat_id` at inbound time should read it from the `pre_gateway_dispatch` payload's `event.source` (`event: MessageEvent`; kwargs documented at `hermes_cli/plugins.py:234`).
...
- `hermes_cli/plugins.py:3387` — `register_hook`; `:5564` — `invoke_hook` (kwarg-filter `:5537-5562`, exception swallow `:5692-5698`)
...
- Gate: `hermes_cli/plugins.py:6831` `_dispatch_pre_tool_call_hooks`; hook emit `:6635`; callers `agent/tool_executor.py:647-649`, `agent/agent_runtime_helpers.py:3475-3476`, `model_tools.py:1435-1436`
