...
**Declaring gated jobs in the compose spec.** A coworker type declares its pre-task jobs under a top-level `cron_jobs:` list, which appends across the `extends` chain (a spine ships fleet-wide jobs; a type adds its own; a duplicate job name across the chain is a render error — names are compared **case-insensitively**, matching the runtime's job-reference resolution, so `Health` and `health` collide). Each entry is `{name, schedule, prompt?, script?, monitor?, no_agent?, skill?, skills?, deliver?}`; the `monitor` alias is normalized to `monitor_url` (for an `http(s)://` value) or `monitor_script` (otherwise). A **skill-backed** job carries `skill` (a single skill name) or `skills` (a name or a list of names, normalized to a unique ordered list exactly as native `hermes cron create --skill` persists it) and runs the agent turn — a job needs a `prompt`, a `script`, **or** a skill. `deliver` (e.g. `bot-chat`, `bot-chat:<profile>`, a platform name) is the delivery target, passed through verbatim — the same values the [delivery options](#delivery-options) above accept. `skill`/`skills`/`deliver` are emitted only when declared, so a job that omits them renders byte-identically. A malformed value (a non-string skill entry, a blank `deliver`) fails the render closed. The render writes each type's jobs to `<profile>/cron/jobs.json`. A rendered `deliver: bot-chat` skill-backed cron is how the fleet ships its recurring supervisor — see [Fleet issue supervision and worktree reaping](./fleet-issue-supervision.md).
