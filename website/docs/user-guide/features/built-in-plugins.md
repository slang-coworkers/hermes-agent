...
A fleet-wide index that answers *who owns this PR?* durably, so a PR created by one worker stays owned by that worker's session — and so per-artifact outcomes and cost are recorded for the whole fleet. One `plugin_db` holds two tables.
