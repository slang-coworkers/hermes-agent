...
| **public** | `<PLATFORM>_ALLOW_ALL_USERS=true` (checked **unconditionally**) or `GATEWAY_ALLOW_ALL_USERS=true` (applies **only when no allowlist is configured**) — **both off for the fleet**. | Admitted with no approval step (tag: per-platform `gateway/authz_mixin.py:679-682`, global `gateway/authz_mixin.py:823`). |
...
An "open" DM policy is not an allow-all. Without an `*_ALLOW_ALL_USERS` flag an open `dm_policy` still falls through to default-deny (tag: `gateway/authz_mixin.py:731-746`); admitting everyone requires the explicit allow-all flag.
:::
